...
# === LIVE-ZIEHUNG (integriert) ===
from datetime import date, timedelta

def letzter_ziehungstag():
    heute = date.today()
    if heute.weekday() in [1, 4]:  # Dienstag/Freitag
        return heute
...
def aktuelle_ziehung_simuliert():
    return {
...
def zeige_live_ziehung():
    daten = aktuelle_ziehung_simuliert()
...
if st.button('📡 Aktuelle Ziehung anzeigen'):
    zeige_live_ziehung()
    ...
    gesamt_gewinn = 0
    waehrung = st.session_state.get("waehrung", "€")
    ...
    if st.session_state.get("beste_tipps"):
        for i, (tipp, sterne) in enumerate(st.session_state.beste_tipps, 1):
            ...
            &#10035;&#65039; <b>Sterntreffer:</b> {sterntreffer} von 2<br>
            💰 <b>Gewinn:</b> {gewinn:.2f} {waehrung}
            </div>
            ''', unsafe_allow_html=True)

        st.markdown(f"<h3 style='color:green;'>💸 Gesamtgewinn: {gesamt_gewinn:.2f} {waehrung}</h3>", unsafe_allow_html=True)
