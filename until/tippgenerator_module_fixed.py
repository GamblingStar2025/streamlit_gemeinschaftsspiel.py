...
def get_cyclic_numbers(history, window=20):
    repeating = []
    for i in range(len(history) - window - 5):
        past = set(n for row in history[i:i+window] for n in row)
        future = set(n for row in history[i+window:i+window+5] for n in row)
        repeating.extend(past.intersection(future))
